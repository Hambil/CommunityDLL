...
//------------------------------------------------------------------------------
void CvDllNetMessageHandler::ResponseMoveSpy(PlayerTypes ePlayer, int iSpyIndex, int iTargetPlayer, int iTargetCity)
{
	CvPlayerAI& kPlayer = GET_PLAYER(ePlayer);
	...
	if(pPlayerEspionage)
	{
		if(iTargetCity == -1)
		{
			...
			gDLL->getInterfaceIFace()->setDirty(EspionageScreen_DIRTY_BIT, true);
		}
		else
		{
