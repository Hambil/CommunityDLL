...
bool EconomicAIHelpers::IsTestStrategy_EarlyExpansion(CvPlayer* pPlayer)
{
	...
	iDesiredCities = iDesiredCities < iMaxCultureCities ? iMaxCultureCities : iDesiredCities; // the MaxCultureCities only changes with bigger map sizes
	...
	if(eGrandStrategy != NO_AIGRANDSTRATEGY)
	{
		if(pPlayer->GetGrandStrategyAI()->GetActiveGrandStrategy() == eGrandStrategy)
		{
			...
			if(GC.getMap().getWorldInfo().GetNumCitiesPolicyCostMod() > 0)
			{
				...
				iMaxCultureCities = (iMaxCultureCities * max(iDefaultNumTiles,GC.getMap().numPlots())) / iDefaultNumTiles;
