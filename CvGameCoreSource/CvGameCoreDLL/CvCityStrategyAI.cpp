...
void CvCityStrategyAI::UpdateFlavorsForNewCity()
{
	...
	for (int iStrategyLoop = 0; iStrategyLoop < GC.getNumEconomicAIStrategyInfos(); iStrategyLoop++)
	{
		...
		if(pStrategy)
		{
			...
			{
				// Is this a Strategy where we DON'T want to be sending the Flavors to a City?
				if (!pStrategy->IsDontUpdateCityFlavors())
				{
					for (iFlavorLoop = 0; iFlavorLoop < GC.getNumFlavorTypes(); iFlavorLoop++)
					{
...
void CvCityStrategyAI::ChooseProduction(bool bUseAsyncRandom, BuildingTypes eIgnoreBldg /* = NO_BUILDING */, UnitTypes eIgnoreUnit/*  = NO_UNIT */)
{
	...
	CvPlayer& thisPlayer = GET_PLAYER(m_pCity->getOwner());

	int iSettlersOnMap = thisPlayer.GetNumUnitsWithUnitAI(UNITAI_SETTLE, true);
	...
	if (eUnitForOperation != NO_UNIT)
	{
		...
		int iOffenseFlavor = thisPlayer.GetGrandStrategyAI()->GetPersonalityAndGrandStrategy((FlavorTypes)GC.getInfoTypeForString("FLAVOR_OFFENSE"));
		...
		if (pkUnitEntry && pkUnitEntry->GetDefaultUnitAIType() == UNITAI_SETTLE)
		{
			...
			else
			{
				...
				int iNumGoodAreas = thisPlayer.GetBestSettleAreas(thisPlayer.GetEconomicAI()->GetMinimumSettleFertility(), iBestArea, iSecondBestArea);
				if (iNumGoodAreas == 0)
				{
					iTempWeight = 0;
				}
		...
		case CITY_BUILDABLE_UNIT:
			{
				...
				if(pkUnitInfo)
				{
					...
					GetCity()->pushOrder(ORDER_TRAIN, eUnitType, eUnitAI, false, false, false, bRush);
				}
			}
			break;
		...
		case CITY_BUILDABLE_BUILDING:
			{
				...
				GetCity()->pushOrder(ORDER_CONSTRUCT, eBuildingType, -1, false, false, false, bRush);
			}
			break;
		...
		case CITY_BUILDABLE_PROJECT:
			{
				...
				GetCity()->pushOrder(ORDER_CREATE, eProjectType, -1, false, false, false, bRush);
			}
			break;
...
void CvCityStrategyAI::LogStrategy(AICityStrategyTypes eStrategy, bool bValue)
{
	if (GC.getLogging() && GC.getAILogging())
	{
		...
		// Strategy Info
		//		szTemp.Format ("Strategy, %s, %d", GC.getAICityStrategyInfo(eStrategy).getType(), bValue);
		strTemp.Format ("%s, %d", GC.getAICityStrategyInfo(eStrategy)->GetType(), bValue);
		strOutBuf = strBaseString + strTemp;
		pLog->Msg(strOutBuf);
	}
}
...
void CvCityStrategyAI::LogPossibleBuilds()
{
	if (GC.getLogging() && GC.getAILogging())
	{
		...
		for (int iI = 0; iI < m_Buildables.size(); iI++)
		{
			...
			{
			case CITY_BUILDABLE_BUILDING:
				strDesc = GC.GetGameBuildings()->GetEntry(buildable.m_iIndex)->GetDescription();
				strTemp.Format("Building, %s, %d, %d", strDesc.GetCString(), m_Buildables.GetWeight(iI), buildable.m_iTurnsToConstruct);
				break;
			case CITY_BUILDABLE_UNIT:
				strDesc = GC.GetGameUnits()->GetEntry(buildable.m_iIndex)->GetDescription();
				strTemp.Format("Unit, %s, %d, %d", strDesc.GetCString(), m_Buildables.GetWeight(iI), buildable.m_iTurnsToConstruct);
				break;
			case CITY_BUILDABLE_PROJECT:
				strDesc = GC.GetGameProjects()->GetEntry(buildable.m_iIndex)->GetDescription();
...
void CvCityStrategyAI::LogSpecializationChange(CitySpecializationTypes eSpecialization)
{
	if (GC.getLogging() && GC.getAILogging())
	{
		...
		YieldTypes eYieldType = GC.getCitySpecializationInfo(eSpecialization)->GetYieldType();
		...
		{
			strYieldString = GC.getYieldInfo(eYieldType)->GetDescription();
		}

		strTemp.Format("NEW SPECIALIZATION: Yield Type = ");

		strOutBuf = strBaseString + strTemp + strYieldString;
		pLog->Msg(strOutBuf);
	}
}
...
bool CityStrategyAIHelpers::IsTestCityStrategy_NeedTileImprovers(AICityStrategyTypes eStrategy, CvCity *pCity)
{
	...
	if (iCurrentNumCities <= 1)
	{
		...
		if (eStrategyKillBarbs != NO_MILITARYAISTRATEGY)
		{
			...
			{
				MilitaryAIStrategyTypes eStrategyCriticalDefense = (MilitaryAIStrategyTypes) GC.getInfoTypeForString("MILITARYAISTRATEGY_EMPIRE_DEFENSE_CRITICAL");
	...
	if (pCity->isCapital())
	{
		...
		if (!kPlayer.isMinorCiv())
		{
			...
			if ((iCitiesPlusSettlers) < 3)
			{
				...
				{
					if (GC.getGame().getGameTurn() > 100 && pCity->GetCityStrategyAI()->IsUsingCityStrategy(eUnderThreat))
					{
						return false;
					}
				}
				...
				if ((iCitiesPlusSettlers == 1 && (GC.getGame().getGameTurn() * 4) > iWeightThreshold) ||
					(iCitiesPlusSettlers == 2 && (GC.getGame().getGameTurn()) > iWeightThreshold))
				{
					return true;
				}
			}
...
bool CityStrategyAIHelpers::IsTestCityStrategy_FirstCultureBuilding(CvCity* pCity)
{
	...
	//if (eStrategyCriticalDefense != NO_MILITARYAISTRATEGY)
	//{
	//	CvMilitaryAI* pMilitaryAI = GET_PLAYER(pCity->getOwner()).GetMilitaryAI();
	//	if (pMilitaryAI->IsUsingStrategy(eStrategyCriticalDefense))
	//	{
	//		return false;
	//	}
