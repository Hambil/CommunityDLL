...
/// Cities that receive a free happiness building
...
void CvPlayerPolicies::Reset()
{
	...
	for (iI = 0; iI < m_pPolicies->GetNumPolicyBranches(); iI++)
	{
		...
		m_paePolicyBlockedBranchCheck[iI] = NO_POLICY_BRANCH_TYPE;
	}
	...
	for(int i = 0; i < iNumPolicies; i++)
	{
		...
		if (m_pabHasPolicy[i] && !IsPolicyBlocked((PolicyTypes)i))
		{
			...
			case POLICYMOD_POLICY_COST_MODIFIER:
				...
				break;
			case POLICYMOD_RELIGION_PRODUCTION_MODIFIER:
				rtnValue += m_pPolicies->GetPolicyEntry(i)->GetReligionProductionModifier();
				break;
...
void CvPlayerPolicies::DoPolicyAI()
{
	...
	{
		if (m_pPlayer->getJONSCulture() >= m_pPlayer->getNextPolicyCost() || m_pPlayer->GetNumFreePolicies() > 0)
		{
			// Choose the policy we want next (or a branch)
			int iNextPolicy = m_pPolicyAI->ChooseNextPolicy(m_pPlayer);
			if (iNextPolicy != NO_POLICY)
			{
				// Low return values indicate a branch has been chosen
